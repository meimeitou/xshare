...
> 重要：Tushare 日线数据通常在交易日 15:00-16:00 入库，**定时任务在交易日 17:00 后触发**。

## 方式一：MCP / Web 自动同步（推荐）

服务启动后，`daily` 按日历在交易日 17:00 入队一次当日增量；并扫描 watermark 缺口自动补洞（最近 30 个交易日内的非 ok 日期）。通过 `sync_job` 或 Web `/sync` 管理：
