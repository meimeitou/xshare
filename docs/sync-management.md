...
3. **水位 + 日历调度**：日线类任务交易日 17:00 触发一次，并用 watermark 补洞。
...
| `daily` | 交易日 17:00 | Tushare `pro.daily` | `stock_daily` | 补数可绕过窗口 |
| `index_daily` | 交易日 17:00 | Tushare `pro.index_daily` | `index_daily` | 按 `index_basic` 循环；补数可绕过窗口 |
| `fund_daily` | 交易日 17:00 | Tushare `pro.fund_daily` | `fund_daily` | 按交易日批量；失败回退按 `etf_basic` |
...
2. 处于 17:00 窗口且对应日线表非空时，入队当日增量 `daily` / `index_daily` / `fund_daily` / `daily_basic`（`days=1`）
