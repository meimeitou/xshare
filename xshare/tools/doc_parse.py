...
async def doc_parse(args: dict) -> str:
    ...
    try:
        ...
        return json.dumps({"error": "mineru-kie-sdk 未安装，请执行: uv pip install mineru-kie-sdk", "retry_same_args": False}, ensure_ascii=False)
    ...
    if not file_path or not Path(file_path).exists():
        return json.dumps({"error": f"文件不存在: {file_path}", "retry_same_args": False}, ensure_ascii=False)
    ...
    try:
        ...
        return json.dumps({"error": f"解析超时（{timeout}s），文档可能过大", "retry_same_args": False}, ensure_ascii=False)
    except Exception as e:
        return json.dumps({"error": f"解析失败: {str(e)}", "retry_same_args": False}, ensure_ascii=False)
    finally:
        # 清理临时文件
        if tmp_file:
            try:
